...
const Reader = ({record,gate,nature}:any) => {
    ...
    <div className="w-4/5 sm:w-3/5 text-start" key={record.id}>
      ...
      {/* {record.hash&&record.file&&<img src={`https://${gate}/ipfs/${record.hash}/${record.file}`} className='w-64'/>}{record.hash&&<p className='py-1'/>} */}
        {record.hash1&&<div className="flex-container">
          {record.hash1&&<img src={`https://${gate}/${nature}/${record.hash1}`} className='w-32'/>}
          {record.hash2&&<img src={`https://${gate}/${nature}/${record.hash2}`} className='w-32'/>}
          {record.hash3&&<img src={`https://${gate}/${nature}/${record.hash3}`} className='w-32'/>}
          {record.hash4&&<img src={`https://${gate}/${nature}/${record.hash4}`} className='w-32'/>}
          {record.hash5&&<img src={`https://${gate}/${nature}/${record.hash5}`} className='w-32'/>}
          {record.hash6&&<img src={`https://${gate}/${nature}/${record.hash6}`} className='w-32'/>}
